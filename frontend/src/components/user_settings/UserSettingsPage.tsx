...
function UserSettingRow({ setting, getString, onSave, onReset, saving }: RowProps) {
    const { t } = useTheme();
    const isBoolean = setting.value_type_key === 'boolean';
    const isInteger = setting.value_type_key === 'integer';
    ...
    const dirty = !isBoolean && JSON.stringify(draft) !== JSON.stringify(setting.effective_value);
                ...
                <Box sx={{ minWidth: 220, flex: 1 }}>
                    ...
                    {!setting.has_override ? (
                        <Chip
                            ...
                            variant="outlined"
                            sx={{ mt: 0.75 }}
                            label={getString('usingDefault', { value: String(setting.global_value) })}
                        />
                    ) : (
                        isInteger && (
