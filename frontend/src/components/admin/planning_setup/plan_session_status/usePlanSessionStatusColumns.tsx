...
interface Params {
    ...
    onEditFieldClick: (row: PlanSessionStatus, field: string, e: React.MouseEvent) => void;
    onRequestSave: (row: PlanSessionStatus, field: string, newValue: string) => void;
    onCancelEdit: () => void;
    updateIsPending: boolean;
    onDeleteClick: (row: PlanSessionStatus) => void;
    deleteIsPending: boolean;
}

export function usePlanSessionStatusColumns({
    getString,
    editingState,
    onEditFieldClick,
    onRequestSave,
    ...
    return [
        textEditCol('key', 'key', 0.6),
        textEditCol('name', 'name', 0.8),
        textEditCol('description', 'description', 1.2),
        {
            field: 'is_active',
