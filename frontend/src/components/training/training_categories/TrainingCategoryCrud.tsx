...
import { fetchTrainingCategories, type TrainingCategory } from './trainingCategoryApi';
...
export function TrainingCategoryCrud() {
    ...
    const [rowToDelete, setRowToDelete] = useState<TrainingCategory | null>(null);
    ...
    const { createMutation, updateMutation, deleteMutation } = useTrainingCategoryMutations({
        setSnackbar,
    ...
    const handleEditFieldClick = useCallback(
        (row: TrainingCategory, field: string, e: React.MouseEvent) => {
            e.stopPropagation();
    ...
    const handleRequestSave = useCallback(
        (row: TrainingCategory, field: string, newValue: string) => {
    ...
    const handleDeleteClick = useCallback((row: TrainingCategory) => {
        setRowToDelete(row);
    }, []);
        ...
        <Box>
            <Box sx={{ display: 'flex', alignItems: 'center', gap: 2, mb: 2 }}>
                <Typography variant="h6" fontWeight={600} sx={{ flex: 1 }}>
                    {getString('trainingCategories') || 'Training Categories'}
                </Typography>
                <Button
                    variant="contained"
                    size="medium"
                ...
                >
                    {cfl(getString('addTrainingCategory')) || 'Add'}
                </Button>
            </Box>
