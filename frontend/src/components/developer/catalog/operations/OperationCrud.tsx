// src/components/admin/operations/OperationCrud.tsx
...
import { fetchOperations, type Operation } from './operationApi.ts';
import { useOperationMutations } from './useOperationMutations.ts';
import { useOperationColumns, type EditingState } from './useOperationColumns.tsx';
import { OperationForm } from './OperationForm.tsx';
import { OperationDeleteDialog } from './OperationDeleteDialog.tsx';
...
export function OperationCrud() {
  ...
  const [rowToDelete, setRowToDelete] = useState<Operation | null>(null);
  ...
  const { createMutation, updateMutation, deleteMutation } = useOperationMutations({
    setSnackbar,
  ...
  if (error) return <Alert severity="error">Failed to load operations</Alert>;

  return (
    <Box>
      <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', mb: 2 }}>
        <Typography variant="h6" fontWeight={600}>
          {cfl(getString('operations')) || 'Operations'}
        </Typography>
        <Button
          size="small"
          variant="outlined"
