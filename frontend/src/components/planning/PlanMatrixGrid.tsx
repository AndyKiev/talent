...
function flattenRow(row: MatrixRow, jobGroups: JobGroupDef[]): FlatRow {
  const out: FlatRow = {
  ...
  for (const jg of jobGroups) {
    ...
    if (config.fact_mode === 'by_status') {
      ...
      out[flatKey(id, 'fact')] = (jgData.fact as number) ?? null;
    } else {
...
function buildColumnDefs(jobGroups: JobGroupDef[]): (ColDef | ColGroupDef)[] {
  ...
  const leaf = (headerName: string, field: string, isPct = false): ColDef => ({
    headerName,
    field,
    flex: isPct ? 0.9 : 0.6,   // numbers are small — give them ~40% less space
    minWidth: isPct ? 56 : 40,
    valueFormatter: isPct ? pctFormatter : numFormatter,
    cellStyle: isPct ? pctCellStyle : numCellStyle,
    sortable: true,
    resizable: true,
...
const HEADER_CSS = `
  .ag-theme-matrix .ag-header-group-cell.hdr-l1,
  .ag-theme-matrix .ag-header-group-cell.hdr-l1 .ag-header-group-cell-label {
    background: ${C.h1Bg} !important;
    color: #ffffff !important;
    font-weight: 700;
    justify-content: center;
  }
  .ag-theme-matrix .ag-header-group-cell.hdr-l2,
  .ag-theme-matrix .ag-header-group-cell.hdr-l2 .ag-header-group-cell-label {
    background: ${C.h2Bg} !important;
    color: #ffffff !important;
    font-weight: 600;
    justify-content: center;
  }
  .ag-theme-matrix .ag-header-cell-label { justify-content: center; }
  .ag-theme-matrix .ag-pinned-left-cols-container .ag-cell:last-child {
