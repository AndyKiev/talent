from typing import Optional, List

from sqlalchemy.exc import IntegrityError
from sqlalchemy.ext.asyncio import AsyncSession

from backend.api_v1.base.base_service import BaseService
from backend.api_v1.base.mutation_response import MutationResponse
from backend.api_v1.talent_audit_status.talent_audit_status_repository import (
    TalentAuditStatusRepository,
)
from backend.api_v1.talent_audit_status.talent_audit_status_schema import (
    TalentAuditStatus as TalentAuditStatusSchema,
    TalentAuditStatusCreate,
    TalentAuditStatusUpdate,
)
from backend.api_v1.employee.employee_schema import EmployeeSchema
from backend.api_v1.talent_audit_status.talent_audit_status_messages import (
    TalentAuditStatusNotFound,
    TalentAuditStatusNotFoundByName,
    TalentAuditStatusNameTaken,
    TalentAuditStatusDeleteError,
)
from backend.api_v1.talent_audit_status.talent_audit_status_messages import (
    TalentAuditStatusDeleteSuccess,
    TalentAuditStatusCreateSuccess,
    TalentAuditStatusUpdateSuccess,
)


class TalentAuditStatusService(BaseService):
    def __init__(
        self,
        repository: TalentAuditStatusRepository,
        user: Optional[EmployeeSchema] = None,
        session: Optional[AsyncSession] = None,
    ):
        super().__init__(repository, user=user, session=session)

    async def get_by_id(self, id: int) -> TalentAuditStatusSchema:
        result = await self.repository.get_by_id(id)
        if not result:
            raise await self._resolve_domain_error(TalentAuditStatusNotFound(id))
        return result

    async def get_talent_audit_statuses(
        self,
        name: Optional[str] = None,
        sort: Optional[str] = None,
    ) -> List[TalentAuditStatusSchema]:
        if name:
            record = await self.get_by_name(
                name, not_found_exc=TalentAuditStatusNotFoundByName
            )
            return [TalentAuditStatusSchema.model_validate(record)]
        records = await self.get_all(sort_json=sort)
        return [TalentAuditStatusSchema.model_validate(r) for r in records]

    async def create_talent_audit_status(
        self, status_in: TalentAuditStatusCreate
    ) -> MutationResponse[TalentAuditStatusSchema]:
        await self.exists_by_name(
            status_in.name, already_exists_exc=TalentAuditStatusNameTaken
        )
        try:
            record = await self.create(status_in)
            schema = TalentAuditStatusSchema.model_validate(record)
            detail = await self._resolve_domain_success(
                TalentAuditStatusCreateSuccess(schema.name)
            )
            return MutationResponse(detail=detail, data=schema)
        except IntegrityError:
            raise await self._resolve_domain_error(
                TalentAuditStatusNameTaken(status_in.name)
            )

    async def update_talent_audit_status(
        self, status_id: int, status_update: TalentAuditStatusUpdate
    ) -> MutationResponse[TalentAuditStatusSchema]:
        if status_update.name:
            await self.exists_by_name(
                status_update.name, already_exists_exc=TalentAuditStatusNameTaken
            )
        try:
            orm_record = await self.get_by_id(status_id)
            updated = await self.update(orm_record, status_update, partial=True)
            schema = TalentAuditStatusSchema.model_validate(updated)
            detail = await self._resolve_domain_success(
                TalentAuditStatusUpdateSuccess(schema.name)
            )
            return MutationResponse(detail=detail, data=schema)
        except IntegrityError:
            raise await self._resolve_domain_error(
                TalentAuditStatusNameTaken(status_update.name)
            )

    async def delete_talent_audit_status(self, status_id: int) -> None:
        record = await self.get_by_id(status_id)
        await self.delete_by_id(
            status_id,
            name=record.name,
            delete_error_exc=TalentAuditStatusDeleteError,
            delete_success_exc=TalentAuditStatusDeleteSuccess,
        )
