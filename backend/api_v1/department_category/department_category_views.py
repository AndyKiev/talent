...
@router.get("", response_model=List[DepartmentCategorySchema])
async def get_department_categories(
    ...
    return await service.get_department_categories(name=name, is_active=is_active, is_main=is_main, sort=sort)


@router.get("/{department_category_id}", response_model=DepartmentCategorySchema)
