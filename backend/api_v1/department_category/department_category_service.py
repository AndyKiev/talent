...
class DepartmentCategoryService(BaseService):
    ...
    async def get_department_categories(
        ...
        if name:
            record = await self.get_by_name(name, not_found_exc=DepartmentCategoryNotFoundByName)
    ...
    async def create_department_category(
        ...
        await self.exists_by_name(category_in.name, already_exists_exc=DepartmentCategoryNameTaken)
        try:
            record = await self.create(category_in)
            schema = DepartmentCategorySchema.model_validate(record)
            detail = await self._resolve_domain_success(DepartmentCategoryCreateSuccess(schema.name))
            return MutationResponse(detail=detail, data=schema)
        except IntegrityError:
            raise await self._resolve_domain_error(DepartmentCategoryNameTaken(category_in.name))

    async def update_department_category(
        self, category_id: int, category_update: DepartmentCategoryUpdate
    ) -> MutationResponse[DepartmentCategorySchema]:
        if category_update.name:
            await self.exists_by_name(category_update.name, already_exists_exc=DepartmentCategoryNameTaken)
        try:
            ...
            detail = await self._resolve_domain_success(DepartmentCategoryUpdateSuccess(schema.name))
            return MutationResponse(detail=detail, data=schema)
        except IntegrityError:
            raise await self._resolve_domain_error(DepartmentCategoryNameTaken(category_update.name))
