
from sqlalchemy.exc import IntegrityError
from sqlalchemy.ext.asyncio import AsyncSession

from backend.api_v1.base.base_service import BaseService
from backend.api_v1.base.mutation_response import MutationResponse
from backend.api_v1.candidate_source.candidate_source_messages import (
    CandidateSourceCreateSuccess,
    CandidateSourceDeleteError,
    CandidateSourceDeleteSuccess,
    CandidateSourceKeyTaken,
    CandidateSourceNotFound,
    CandidateSourceUpdateSuccess,
)
from backend.api_v1.candidate_source.candidate_source_repository import (
    CandidateSourceRepository,
)
from backend.api_v1.candidate_source.candidate_source_schema import (
    CandidateSource as CandidateSourceSchema,
)
from backend.api_v1.candidate_source.candidate_source_schema import (
    CandidateSourceCreate,
    CandidateSourceUpdate,
)
from backend.api_v1.employee.employee_schema import EmployeeSchema


class CandidateSourceService(BaseService):
    def __init__(
        self,
        repository: CandidateSourceRepository,
        user: EmployeeSchema | None = None,
        session: AsyncSession | None = None,
    ):
        super().__init__(repository, user=user, session=session)

    async def get_by_id(self, id: int) -> CandidateSourceSchema:
        result = await self.repository.get_by_id(id)
        if not result:
            raise await self._resolve_domain_error(CandidateSourceNotFound(id))
        return result

    async def get_candidate_sources(
        self, sort: str | None = None
    ) -> list[CandidateSourceSchema]:
        if sort:
            records = await self.get_all(sort_json=sort)
        else:
            records = await self.get_all(sort=["sort_order", "id"])
        return [CandidateSourceSchema.model_validate(r) for r in records]

    async def create_candidate_source(
        self, source_in: CandidateSourceCreate
    ) -> MutationResponse[CandidateSourceSchema]:
        existing = await self.repository.get_by_field("key", source_in.key)
        if existing:
            raise await self._resolve_domain_error(
                CandidateSourceKeyTaken(source_in.key)
            )
        try:
            record = await self.create(source_in)
            schema = CandidateSourceSchema.model_validate(record)
            detail = await self._resolve_domain_success(
                CandidateSourceCreateSuccess(schema.key)
            )
            return MutationResponse(detail=detail, data=schema)
        except IntegrityError:
            raise await self._resolve_domain_error(
                CandidateSourceKeyTaken(source_in.key)
            )

    async def update_candidate_source(
        self, source_id: int, source_update: CandidateSourceUpdate
    ) -> MutationResponse[CandidateSourceSchema]:
        if source_update.key:
            existing = await self.repository.get_by_field("key", source_update.key)
            if existing and existing.id != source_id:
                raise await self._resolve_domain_error(
                    CandidateSourceKeyTaken(source_update.key)
                )
        try:
            orm_record = await self.get_by_id(source_id)
            updated = await self.update(orm_record, source_update, partial=True)
            schema = CandidateSourceSchema.model_validate(updated)
            detail = await self._resolve_domain_success(
                CandidateSourceUpdateSuccess(schema.key)
            )
            return MutationResponse(detail=detail, data=schema)
        except IntegrityError:
            raise await self._resolve_domain_error(
                CandidateSourceKeyTaken(source_update.key)
            )

    async def delete_candidate_source(self, source_id: int) -> None:
        record = await self.get_by_id(source_id)
        await self.delete_by_id(
            source_id,
            name=record.key,
            delete_error_exc=CandidateSourceDeleteError,
            delete_success_exc=CandidateSourceDeleteSuccess,
        )
