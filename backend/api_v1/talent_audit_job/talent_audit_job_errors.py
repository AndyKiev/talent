...
class TalentAuditJobDuplicateJob(DomainError):
    message_key = "talentAuditJobDuplicateJob"

    def __init__(self, target_job_id: int) -> None:
        self.template_vars = {"targetJobId": target_job_id}
        self.fallback = f"Job {target_job_id} is already added to this audit"
        super().__init__(self.fallback)
