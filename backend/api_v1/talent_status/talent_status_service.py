...
class TalentStatusService(BaseService):
    ...
    async def get_talent_statuses(
        ...
        if name:
            record = await self.get_by_name(name, not_found_exc=TalentStatusNotFoundByName)
    ...
    async def create_talent_status(
        ...
        # Guard unique key
        existing_key = await self.repository.get_by_field("key", status_in.key, case_insensitive=True)
        if existing_key:
        ...
        try:
            ...
            detail = await self._resolve_domain_success(TalentStatusCreateSuccess(schema.name))
    ...
    async def update_talent_status(
        ...
        if status_update.key:
            ...
            if existing_key:
                raise await self._resolve_domain_error(TalentStatusKeyTaken(status_update.key))
        try:
            ...
            detail = await self._resolve_domain_success(TalentStatusUpdateSuccess(schema.name))
            return MutationResponse(detail=detail, data=schema)
        except IntegrityError:
            raise await self._resolve_domain_error(TalentStatusKeyTaken(status_update.key or ""))
