...
class DepartmentTypeParentalLinkAlreadyExists(AlreadyExistsError):
    ...
    def __init__(self, child_id: int, parent_id: int) -> None:
        ...
        self.fallback = f"Link between child {child_id} and parent {parent_id} already exists"
        super().__init__("DepartmentTypeParentalLink", "child_parent", f"{child_id}-{parent_id}")
