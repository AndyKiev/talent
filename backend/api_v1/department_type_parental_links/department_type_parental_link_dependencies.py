...
def get_department_type_parental_link_service(
    repository: DepartmentTypeParentalLinkRepository = Depends(
        get_department_type_parental_link_repository
    ),
    user: EmployeeSchema = Depends(get_current_active_auth_user),
    session: AsyncSession = Depends(db_helper.session_getter),
) -> DepartmentTypeParentalLinkService:
    return DepartmentTypeParentalLinkService(
...
async def get_link_by_id(
    link_id: int,
    service: DepartmentTypeParentalLinkService = Depends(
