...
class DepartmentTypeParentalLink(IntIdPkMixin, TimestampMixin, Base):
    ...
    __table_args__ = (
        UniqueConstraint("child_id", "parent_id", name="uq_dept_type_parental_link_child_parent"),
    )

    child_id: Mapped[int] = mapped_column(Integer, ForeignKey("department_types.id"), nullable=False)
    parent_id: Mapped[int] = mapped_column(Integer, ForeignKey("department_types.id"), nullable=False)
