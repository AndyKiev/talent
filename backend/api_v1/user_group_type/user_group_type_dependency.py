...
async def get_user_group_type_service(
    ...
    """
    Build UserGroupTypeService with session + user so that _translate()
    can resolve messages in the user's preferred language.
    """
...
async def user_group_type_by_id(
    ...
    """Resolve user group type by ID → schema. Raises UserGroupTypeNotFound (→ 404) if missing."""
