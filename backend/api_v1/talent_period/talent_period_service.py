...
class TalentPeriodService(BaseService):
    ...
    async def get_talent_periods(
        ...
        if name:
            record = await self.get_by_name(name, not_found_exc=TalentPeriodNotFoundByName)
    ...
    async def create_talent_period(
        ...
        await self.exists_by_name(period_in.name, already_exists_exc=TalentPeriodNameTaken)
        try:
            record = await self.create(period_in)
            schema = TalentPeriodSchema.model_validate(record)
            detail = await self._resolve_domain_success(TalentPeriodCreateSuccess(schema.name))
            return MutationResponse(detail=detail, data=schema)
        except IntegrityError:
            raise await self._resolve_domain_error(TalentPeriodNameTaken(period_in.name))

    async def update_talent_period(
        self, period_id: int, period_update: TalentPeriodUpdate
    ) -> MutationResponse[TalentPeriodSchema]:
        if period_update.name:
            await self.exists_by_name(period_update.name, already_exists_exc=TalentPeriodNameTaken)
        try:
            ...
            detail = await self._resolve_domain_success(TalentPeriodUpdateSuccess(schema.name))
            return MutationResponse(detail=detail, data=schema)
        except IntegrityError:
            raise await self._resolve_domain_error(TalentPeriodNameTaken(period_update.name or ""))
