...
def test_crud_lifecycle(api: httpx.Client):
    # Job groups require a job_group_type_id FK — create one first.
    type_name = unique_name("jgt_for_group")
    type_resp = api.post(TYPE_PATH, json={
        "name": type_name,
        "key": unique_name("jgt_key"),
    })
    assert type_resp.status_code == 201, type_resp.text
    type_id = type_resp.json()["data"]["id"]

    group_name = unique_name("job_group")
    group_id = None
    try:
        # CREATE
        created = api.post(PATH, json={
            "name": group_name,
            "key": unique_name("jg_key"),
            "job_group_type_id": type_id,
        })
